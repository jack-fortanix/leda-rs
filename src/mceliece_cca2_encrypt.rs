...
fn encrypt_McEliece(pk: &LedaPublicKey, ptx: &[DIGIT], err: &[DIGIT]) -> Vec<DIGIT> {
    ...
    for i in 0..(N0 - 1) {
        ...
        gf2x_mod_add_2(
            &mut codeword[(N0 - 1) * NUM_DIGITS_GF2X_ELEMENT..N0 * NUM_DIGITS_GF2X_ELEMENT],
            &saux,
        );
    }
    for i in 0..N0 {
        gf2x_mod_add_2(
            &mut codeword[i * NUM_DIGITS_GF2X_ELEMENT..(i + 1) * NUM_DIGITS_GF2X_ELEMENT],
...
fn bytestream_into_poly_seq(
    ...
    for polyIdx in 0..numPoly {
        for exponent in 0..P {
            ...
            gf2x_set_coeff(
                &mut polySeq[NUM_DIGITS_GF2X_ELEMENT * polyIdx..],
                exponent,
                buffer,
            );
        }
...
pub fn encrypt_Kobara_Imai(pk: &LedaPublicKey, msg: &[u8]) -> Result<Vec<u8>> {
    ...
    let prngSequence = deterministic_random_byte_generator(&secretSeed, paddedSequenceLen)?;
    ...
    bytestream_into_poly_seq(&mut informationWord, N0 - 1, &mut iwordBuffer)?;
